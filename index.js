...
const client = new WAConnection()
   ...
   console.log(color('♥','white'),color('♥','red'),color('♥','white'),color('SQAN CODENYA!!','white'),color('YOU','red'),color('TUBE','white'),color('Mr.A43G','yellow'))
})
...
break
	...
	case 'hobby':
		...
		if (isLimit(sender)) return reply(ind.limitend(pusname)) 
               ...
               if (anu.error) return reply(anu.error)
                 infomp3 = `*「❗」Lagu Ditemukan*\n➸ Judul : ${anu.result.title}\n➸ Durasi : ${anu.result.duration}\n➸ Size : ${anu.result.size}\n\n*[WAIT] Proses Dumlu Yakan[https://saweria.co/ZakiAhsan]*`
        	...
        	break
				...
				if (!isQuotedSticker) return reply('Reply/tag sticker !')
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						...
						if (err) return reply(ind.stikga())
						buffer = fs.readFileSync(ran)
						client.sendMessage(from, buffer, image, {quoted: mek, caption: 'tuh dh jadi Donasi Kak https://saweria.co/ZakiAhsan  '})
						fs.unlinkSync(ran)
					})
					await limitAdd(sender)
					break
